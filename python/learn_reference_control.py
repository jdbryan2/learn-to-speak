...
for index in index_list:
    ...
    if not is_init:
        ...
        action_fl = data['action_hist'][:, 0].reshape((A_dim, 1))
        ...
        #state_fl = np.append(state_fl, data['state_hist'][:A_dim, -1].reshape((A_dim, 1)), axis=1)
        action_fl = np.append(action_fl, data['action_hist'][:, 0].reshape((A_dim, 1)), axis=1)
        #action_fl = np.append(action_fl, data['action_hist'][:, -1].reshape((A_dim, 1)), axis=1)
...
plt.imshow(_A_)
plt.figure()
plt.plot(np.dot(_A_, data['action_hist']).T, 'g--')
plt.plot(np.dot(_A_.T, data['action_hist']).T, 'r--')
plt.plot(data['state_hist'].T, 'b-')
plt.show()
